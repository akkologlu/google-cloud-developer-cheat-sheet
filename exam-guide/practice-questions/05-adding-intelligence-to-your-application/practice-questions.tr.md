...
A. Natural Language AI, çünkü duygu (sentiment) tespit edebilir
B. Vision AI, yüz tespiti (face detection) yeteneğini kullanarak
...
A. Vision AI'ın landmark (nirengi noktası) tespiti
B. Özellikle Sfenks fotoğrafları üzerinde eğitilmiş özel bir AutoML görüntü sınıflandırıcısı
C. Bir görsel açıklaması (caption) üreten generative AI
...
B. Cloud Translation API, metni istek anında dinamik olarak çevirerek
C. Document AI
...
A. Hayır — Natural Language AI, metinden duygu ve niyet çıkarmak için özel olarak tasarlanmıştır ve bu dar, iyi tanımlı görev için daha uygundur
B. Evet — metinden duygu ve niyeti sadece genel amaçlı bir LLM (generative AI) belirleyebilir
...
A. Vision AI, çünkü zaten görsellerde logo tespiti yapabiliyor
B. Video AI (Video Intelligence API), varlıkları çekim, kare ya da video düzeyinde, zamanlamayla birlikte açıklayan (annotate eden) servis
C. Document AI
D. Natural Language AI
...
B. Document AI, yapılandırılmamış belgeleri yapılandırılmış veriye dönüştürür
C. Translation AI
...
B. Agent Platform AutoML, kendi verileriyle hiç kod yazmadan özel bir model eğiterek
C. TensorFlow ya da PyTorch ile inşa edilmiş özel bir model
D. Büyük dil modelini (LLM) kendi tablo verileri üzerinde ince ayar (fine-tune) yaparak
...
B. Önceden eğitilmiş bir API
C. TensorFlow ya da PyTorch kullanarak özel bir model inşa edip eğitmek
...
A. Dar makine öğrenmesi — makine bu kuralı etiketlenmiş kedi fotoğraflarından öğrendi
B. Geleneksel programlama — kuralı bir insan elle yazdı; bu, kuralları veriden öğrenen ML modellerinden farklı, daha eski bir yaklaşımdır
...
C. LLM, foundation model'in en popüler türüdür ve yalnızca metin verisiyle eğitilir; foundation model'ler genel olarak görüntü ya da kod gibi başka veri tipleriyle de eğitilebilir
D. Foundation model kullanılmadan önce her zaman ince ayar (fine-tune) yapılır, LLM ise her zaman hiç ince ayar yapılmadan doğrudan kullanılır
...
A. Evet — "büyük" yalnızca parametre sayısını ifade eder
B. Evet — "büyük" yalnızca eğitim veri kümesinin boyutunu ifade eder
C. Hayır — "büyük", hem eğitim veri kümesinin devasa boyutunu (bazen petabayt ölçeğinde) hem de model eğitim sırasında öğrendiği milyarlarca-trilyonlarca parametreyi birlikte ifade eder
...
B. Basit bir ML sinyalinin (hareket algılandı mı, algılanmadı mı), doğrudan bir zamanlama/iş mantığı kuralıyla ve olay güdümlü mesajlaşmayla birleştirildiğinde, karmaşık bir model olmadan gerçek bir problemi çözebileceğini
C. Konferans odası kamera görüntüsünün kare kare analiz edilmesi için Video AI kullanılması gerektiğini
...
C. Kod açıklama (code explanation) — var olan kodun ne yaptığını ve nasıl yaptığını açıklama
D. Dokümantasyon üretimi — son değişiklikler için sürüm notları yazma

---

## Cevap Anahtarı ve Açıklamalar

**1. Doğru cevap: B — Vision AI, yüz tespiti (face detection) yeteneğini kullanarak**
...
**2. Doğru cevap: A — Vision AI'ın landmark (nirengi noktası) tespiti**
...
**3. Doğru cevap: B — Cloud Translation API, metni istek anında dinamik olarak çevirerek**
...
**4. Doğru cevap: A — Hayır, Natural Language AI bu dar, iyi tanımlı görev için özel olarak tasarlanmıştır**
...
**5. Doğru cevap: B — Video AI (Video Intelligence API)**
...
**7. Doğru cevap: B — Agent Platform AutoML**
...
**8. Doğru cevap: C — TensorFlow ya da PyTorch kullanarak özel bir model inşa edip eğitmek**
...
**9. Doğru cevap: B — Geleneksel programlama**
...
**13. Doğru cevap: C**
Modül, LLM bağlamında "büyük" kelimesini aynı anda iki şeyi kapsayacak şekilde tanımlıyor: eğitim veri kümesinin devasa boyutu (bazen petabayt ölçeğinde) ve modelin eğitim sırasında öğrendiği milyarlarca-trilyonlarca parametre. A ve B seçenekleri, doğru tanımın yarısını yakaladıkları için cazip görünüyor, ama hiçbiri tek başına eksiksiz değil.
...
**15. Doğru cevap: C — Kod açıklama (code explanation)**
