...
A. Natural Language AI, since it can detect sentiment
B. Vision AI, using its face detection capability
...
A. Vision AI's landmark detection
B. A custom AutoML image classifier trained specifically on Sphinx photos
C. Generative AI producing an image caption
...
B. Cloud Translation API, translating text dynamically at request time
C. Document AI
...
A. No — Natural Language AI is purpose-built to extract sentiment and intent from text and is the better fit for this narrow, well-defined task
B. Yes — only a general-purpose LLM (generative AI) can determine sentiment and intent from text
...
A. Vision AI, since it already detects logos in images
B. Video AI (Video Intelligence API), which annotates entities at the shot, frame, or video level along with timing
C. Document AI
D. Natural Language AI
...
B. Document AI, which converts unstructured documents into structured data
C. Translation AI
...
B. Agent Platform AutoML, training a custom model on their own data with no code
C. A custom model built with TensorFlow or PyTorch
D. Fine-tuning a large language model on their spreadsheet data
...
B. A pre-trained API
C. Build and train a custom model using TensorFlow or PyTorch
...
A. Narrow machine learning — the machine learned this rule from labeled cat photos
B. Traditional programming — a human wrote the rule by hand; this is a different, earlier approach than ML models that learn rules from data
...
C. An LLM is the most popular type of foundation model and is trained only on text data; foundation models in general can also be trained on other data types, such as images or code
D. A foundation model is always fine-tuned before use, while an LLM is always used directly without fine-tuning
...
A. Yes — "large" refers exclusively to parameter count
B. Yes — "large" refers exclusively to the size of the training dataset
C. No — "large" refers to both the massive scale of the training dataset (sometimes petabytes) and the huge number of parameters (billions to trillions)
...
B. That a simple ML signal (motion detected or not), combined with straightforward timing/business logic and event-driven messaging, can solve a real problem without a complex model
C. That Video AI must be used to analyze the conference room camera feed frame by frame
...
C. Code explanation — describing what existing code does and how it does it
D. Documentation generation — writing release notes for recent changes

---

## Answer Key & Explanations

**1. Correct: B — Vision AI, using its face detection capability**
...
**2. Correct: A — Vision AI's landmark detection**
...
**3. Correct: B — Cloud Translation API, translating text dynamically at request time**
...
**4. Correct: A — No, Natural Language AI is purpose-built for this narrow, well-defined task**
...
**5. Correct: B — Video AI (Video Intelligence API)**
...
**7. Correct: B — Agent Platform AutoML**
...
**8. Correct: C — Build and train a custom model using TensorFlow or PyTorch**
...
**9. Correct: B — Traditional programming**
...
**13. Correct: C**
The module defines "large" in LLM as covering two things at once: the massive scale of the training dataset (sometimes petabytes) and the huge number of parameters (billions to trillions) the model learns during training. Options A and B are each tempting because they capture half of the correct definition, but neither alone is complete.
