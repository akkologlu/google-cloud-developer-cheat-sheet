...
A. Deployment, because it keeps a fixed number of replicas running
B. StatefulSet, because it provides stable identity and per-replica persistent storage
...
B. A Cloud Run job, triggered on a schedule with Cloud Scheduler
C. A GKE Deployment scaled to zero replicas when idle
D. A Compute Engine preemptible VM with a cron job
...
B. Continuous deployment from a GitHub repository
C. Container-based deployment, supplying your own pre-built image
...
A. Container-based deployment with a manually built image
B. Source-based deployment, letting Google Cloud buildpacks build the image
...
B. No, Cloud Run functions are deployed as Cloud Run services under the hood
C. Yes, functions run only on App Engine standard environment
D. No, Cloud Run functions require a dedicated GKE cluster
...
B. Preemptible (Spot) VMs, which offer at least 60% off standard pricing but can be terminated by Google
C. GKE Autopilot with vertical pod autoscaling
...
A. Compute Engine VM with a GPU attached directly
B. GKE Standard with a GPU-enabled node pool
C. Cloud Run with GPU support
...
D. Cloud Run functions with a custom trigger
...
B. Cloud Run, because it's the recommended platform for new serverless services, offering more flexibility and faster scaling
C. Either is equally recommended by Google for new projects
...
A. GKE Standard, for maximum configuration flexibility
B. Compute Engine, for full control over the environment
C. Cloud Run, since it requires no infrastructure or node management
...
**3. Correct answer: B** — StatefulSet.
...
**4. Correct answer: B** — Cloud Run job.
...
**5. Correct answer: B** — Cloud Run service.
...
**6. Correct answer: C** — Container-based deployment.
...
**9. Correct answer: B** — Preemptible (Spot) VMs.
...
**11. Correct answer: C** — Cloud Run with GPU support.
...
**12. Correct answer: C** — Google Kubernetes Engine (GKE).
