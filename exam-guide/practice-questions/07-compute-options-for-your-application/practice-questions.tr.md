...
A. Deployment, çünkü sabit sayıda replica'nın çalışır durumda kalmasını sağlar
B. StatefulSet, çünkü kararlı kimlik ve replica başına kalıcı depolama sağlar
...
B. Cloud Scheduler ile bir zamanlamada tetiklenen bir Cloud Run job'ı
C. Boştayken sıfır replica'ya ölçeklenen bir GKE Deployment
D. Cron job içeren bir Compute Engine preemptible VM'i
...
B. Bir GitHub deposundan sürekli dağıtım
C. Kendi önceden build edilmiş imajını sağladığın container tabanlı dağıtım
...
A. Elle build edilmiş bir imajla container tabanlı dağıtım
B. Google Cloud buildpack'lerinin imajı build etmesine izin veren kaynak kod tabanlı dağıtım
...
B. Hayır, Cloud Run functions arka planda Cloud Run service'leri olarak dağıtılır
C. Evet, fonksiyonlar sadece App Engine standard environment'ında çalışır
D. Hayır, Cloud Run functions özel bir GKE cluster'ı gerektirir
...
B. Standart fiyatlandırmaya göre en az %60 indirim sunan ama Google tarafından sonlandırılabilen Preemptible (Spot) VM'ler
C. Dikey pod otomatik ölçeklendirmeli GKE Autopilot
...
A. GPU'nun doğrudan bağlandığı bir Compute Engine VM'i
B. GPU etkin bir node pool'a sahip GKE Standard
C. GPU destekli Cloud Run
...
D. Özel bir tetikleyiciye sahip Cloud Run functions
...
B. Cloud Run, çünkü yeni serverless servisler için önerilen platform — daha fazla esneklik ve daha hızlı ölçekleme sunuyor
C. Google, yeni projeler için ikisini de eşit derecede önerir
...
A. Maksimum yapılandırma esnekliği için GKE Standard
B. Ortam üzerinde tam kontrol için Compute Engine
C. Cloud Run, çünkü hiçbir altyapı ya da node yönetimi gerektirmez
...
**5. Doğru cevap: B** — Cloud Run service.
...
**6. Doğru cevap: C** — Container tabanlı dağıtım.
...
**9. Doğru cevap: B** — Preemptible (Spot) VM'ler.
...
**11. Doğru cevap: C** — GPU destekli Cloud Run.
...
**12. Doğru cevap: C** — Google Kubernetes Engine (GKE).
