...
B. The team should be graphing saturation instead of latency during incidents.
C. The team should be graphing traffic instead of latency during incidents.
D. The dashboard is blending successful and failed request latency into a single average, which hides the real problem. Latency should be measured separately for successful and failed requests.
...
A. Incorrect — errors include explicit error codes, successful-looking responses with incorrect content, and violations of a promised policy such as a latency SLA.
B. Correct — errors only exist when the server returns an explicit HTTP error status code.
...
B. Saturation should only ever be measured using memory utilization, never CPU.
C. Systems can begin degrading in performance well before reaching 100% utilization, so utilization targets should be set conservatively based on how the system actually behaves, not at the 100% ceiling.
...
B. The Error Reporting Writer IAM role.
C. A Cloud Monitoring uptime check.
D. A log-based alert, since it triggers a notification the moment a matching log pattern occurs.
...
A. A log-based metric, since it's designed for counting occurrences over time, observing trends, and alerting once a threshold is crossed.
B. A log-based alert.
...
B. Turn on Log Analytics — it will automatically infer severity levels from unstructured text lines without any format change.
C. Switch to structured (JSON) logging: emit a single-line serialized JSON object with a `severity` field and a custom `component` field. These land in `jsonPayload` and become queryable.
...
B. This only happens when Cloud Trace is not enabled on the cluster.
C. Cluster events are stored in Cloud Profiler rather than Cloud Logging, so the SRE was looking in the wrong tool.
D. GKE does not persist this data indefinitely: container logs disappear once the pod is removed, system logs are periodically cleared, and cluster events are removed after one hour. Sending logs to Cloud Logging preserves them for as long as needed.
...
A. Managed collectors, which are recommended for all Kubernetes environments (including GKE) because the Kubernetes operator handles running Prometheus for you.
B. Self-deployed collectors, since they are a drop-in replacement for the standard Prometheus binary.
...
B. No — stack traces must first be sent to Cloud Trace before Error Reporting can use them.
C. Yes — Error Reporting can also infer errors by scanning log entries for common text patterns, such as stack traces, without requiring explicit API calls.
...
B. Cloud Profiler must be enabled instead; it is the only tool that can show database query latency.
C. Switch to Cloud Logging log-based metrics to measure the database call's latency.
D. Automatic tracing on Cloud Run only captures the timing of inbound and outbound HTTP requests, not latency inside the service. Instrumenting the application — for example with OpenTelemetry and the Cloud Trace exporter — is required to see internal spans like the database query.
...
A. The request from start to finish is the trace; each suboperation, such as the RPC call or the database query, is a span. A trace consists of one or more spans.
B. Each RPC call and each database query is its own separate trace; the overall request has no name of its own.
...
B. Log Analytics, by writing a SQL query against CPU-related log entries.
C. Cloud Profiler, since it continuously attributes CPU and memory usage to source code across all production instances using low-overhead statistical sampling rather than tracking every single operation.
...
**6. Correct answer: A.**
Counting occurrences, alerting once a threshold is crossed, and observing longer-term trends are exactly what log-based metrics are designed for. A log-based alert (B) is the tempting distractor because it also involves "getting notified," but it's built for immediate pattern matches, not counting/trend analysis.

**7. Correct answer: C.**
Text logs have no `severity` field and are hard to search reliably; structured (JSON) logs place data in `jsonPayload`, let you set `severity` explicitly, and make custom fields like `component` queryable. Log Analytics (B) is a tempting distractor because it does let you run SQL over logs, but it can't invent severity levels or structured fields out of raw unstructured text — the format has to change at the source.
...
**14. Correct answer: A.**
A trace describes the completion of a single overall operation, and a span describes the completion of a suboperation within that trace — a trace always contains one or more spans. Swapping the definitions (C) or treating every suboperation as its own unrelated trace (B) are the common mix-ups.
