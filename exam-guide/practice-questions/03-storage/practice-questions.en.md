...
1. PixShare, a photo- and video-sharing startup, lets users upload photos and short videos up to several gigabytes each. The app needs to serve these files over HTTP from anywhere in the world, and occasionally a mobile client needs to fetch only a small byte range of a large video (for example, to preview a thumbnail frame) without downloading the whole file. Where should PixShare store these uploads?
   ...
   B. Bigtable, because it handles high-throughput writes
   C. Cloud Storage, because it is designed for large, unstructured objects accessed over HTTP
...
2. A chat application needs to sync messages to mobile and web clients in real time, keep working when a user's device goes offline and sync automatically once reconnected, and store a flexible, nested structure for messages and threads that may evolve as the app grows. Which database best fits this application?
   A. Firestore, because it offers real-time sync, offline support, and a flexible document model
   B. Bigtable, because it handles massive write throughput
   C. BigQuery, because it can scale to any data size
...
3. An industrial IoT platform ingests sensor readings from millions of devices, tens of thousands of writes per second, and needs to look up the latest reading for a given device ID in under 10 milliseconds. The team also wants to reuse existing Apache HBase-based tooling with minimal changes. Which service fits?
   ...
   B. Firestore, because it is a fully managed NoSQL database
   C. Cloud SQL, using a heavily indexed table
   D. Bigtable, because it offers sub-10ms key-value lookups at massive scale and is HBase API-compatible
...
6. A bank's PostgreSQL-based ledger system needs to keep processing thousands of transactional writes per second (deposits, withdrawals) while simultaneously running real-time analytical reports on that same data, without exporting it to a separate warehouse or leaving the PostgreSQL ecosystem. Which service fits?
   A. AlloyDB, whose Columnar Engine accelerates analytical queries while preserving transactional performance
...
7. A global payments platform operates across North America, Europe, and Asia. It requires relational data with full ACID transactions, strong consistency worldwide (a write in one region must be immediately visible everywhere), horizontal scalability as transaction volume grows, and an SLA no lower than 99.999%. Which database should they choose?
   ...
   C. Bigtable, for its massive scale
   D. Spanner, built for global strong consistency and horizontal scalability
...
10. A mobile game already uses Redis in production for a real-time leaderboard built on sorted sets, and the team wants a fully managed drop-in replacement with no application code changes. Which Memorystore option should they choose?
    A. Memorystore for Redis, which is fully protocol-compatible and supports richer data structures like sorted sets
    B. Memorystore for Memcached, since it's simpler
...
11. During a design review, an engineer proposes storing user shopping-cart balances only in Memorystore, with no other backing database, arguing it's fast enough and avoids extra components. What is the main risk of this design?
    ...
    B. Memorystore cannot be secured with IAM or VPC
    C. Memorystore cannot handle the required throughput
    D. Memorystore is an in-memory cache, not a durable source of truth — cart data could be lost and should live in a persistent database
...
13. A team's Cloud SQL instance is approaching its maximum storage capacity for a single instance, and data volume keeps growing. What is the recommended way to scale beyond a single instance's limit?
    ...
    B. Compress all existing data to fit within one instance permanently
    C. Shard or partition the data across multiple database instances, since size limits apply per instance, not per architecture
...
14. A video-streaming feature needs to let users seek to any point in a multi-gigabyte video file and start playback immediately, without downloading the entire file first. The videos are stored as objects in Cloud Storage. Which capability enables this?
    A. A ranged GET request, which retrieves only the requested byte range of the object
    B. Cloud Storage does not support partial downloads; the whole object must be fetched
...
15. An analytics team currently runs Apache HBase on self-managed infrastructure and wants to migrate to a managed Google Cloud service with minimal code changes. They also need to resize their cluster during traffic spikes (like a seasonal sale) without any service downtime. Which service fits both requirements?
    ...
    B. BigQuery, since it is serverless
    C. Cloud SQL, using vertical scaling
    D. Bigtable, which is HBase API-compatible and supports seamless cluster resizing with no downtime
...
2. **A — Firestore.** Firestore's document/collection model, strong consistency, and built-in real-time sync plus offline support are exactly designed for mobile/web apps like chat. Bigtable (B) is the classic trap — both are NoSQL, but Bigtable targets billions of rows with sub-10ms single-key lookups, not hierarchical documents with real-time offline sync.

3. **D — Bigtable.** A sparse, massive table with sub-10ms lookups and HBase API compatibility is Bigtable's exact profile. BigQuery (A) is the name-similarity trap — it's an analytical data warehouse, not built for millisecond operational lookups.
...
6. **A — AlloyDB.** AlloyDB's compute/storage separation and Columnar Engine deliver up to 100x faster analytics with no penalty to transactional throughput — the definition of HTAP. Cloud SQL (B) is the trap since it's also managed PostgreSQL, but it doesn't offer AlloyDB's analytical acceleration for a mixed transactional-plus-analytical workload.

7. **D — Spanner.** Strong consistency, horizontal scale, relational data, global reach, and a 99.999% SLA together point unambiguously to Spanner — no other service combines all of these. Cloud SQL's cross-region read replicas (A) only offer near-real-time, not truly strongly-consistent, global reads.
...
13. **C — Shard or partition across multiple instances.** Storage limits are per database instance; splitting data across multiple instances is the standard way to grow beyond a single instance's ceiling without abandoning the service. Option A overstates the limit as an architectural dead end, which the module explicitly says it is not.

14. **A — Ranged GET request.** Cloud Storage supports ranged GET requests, letting a client request a specific byte range of an object — exactly what's needed to seek into a large video without downloading it all. Option B is the trap for anyone who assumes object storage only supports whole-object downloads.

15. **D — Bigtable.** Bigtable's HBase API compatibility eases the migration, and its seamless scaling model applies cluster configuration changes instantly with no downtime during resize. BigQuery (B) is the recurring name trap — it's serverless, but it's an analytical warehouse, not an HBase-compatible operational store.
