...
A. Olay kuyruğu olarak bir Pub/Sub konusu (topic), mesaj kuyruğu olarak bir Eventarc tetikleyicisi
B. Olay kuyruğu olarak bir Eventarc tetikleyicisi, mesaj kuyruğu olarak bir Pub/Sub konusu (topic)
...
B. E-posta servisi yalnızca ihtiyaç duyduğu `name` ve `email` alanlarını okur, yükteki diğer alanları yok sayar
C. Müşteri servisi, e-posta servisi onu tüketmeye başladıktan sonra yükünü asla değiştirmemelidir
D. E-posta servisi müşteri servisini iki kez çağırmalıdır — biri isim/e-posta için, biri toplam alan sayısının eşleştiğini kontrol etmek için
...
B. Durum 1: üstel geri çekilmeyle yeniden dene. Durum 2: devre kesiciyi aç ve servis toparlanana kadar trafik göndermeyi durdur
C. Her iki durumda da: yeniden denemeler ucuz olduğu için hiç geri çekilme olmadan hemen ve tekrar tekrar dene
...
A. Kişiselleştirilmiş fiyatlandırma verisi için Cloud CDN, statik varlıklar için Memorystore
B. Kişiselleştirilmiş fiyatlandırma verisi için Memorystore, statik varlıklar için Cloud CDN
...
B. Eski sistemi arkada tutup modern API'ler sunan bir API ağ geçidi/cephesi olarak Apigee
C. Her tüketicinin eski protokolü kendisinin uygulayacağı doğrudan bir ağ yolu
D. Proxy katmanı olmayan bir Compute Engine load balancer
...
B. Loglarını standart çıktıya (stdout) yazmak ve platformun bunları toplayıp merkezileştirmesine izin vermek
C. Log girişlerini uygulama kodundan doğrudan bir Cloud Storage bucket'ına yazmak
...
B. Canary sürümü
C. Strangler (boğucu) deseni
D. Devre kesici (circuit breaker) deseni
...
A. Cloud Shell — ücretsiz ve tarayıcı tabanlı olduğu için bir ekibin günlük geliştirme ortamı olarak yeterlidir
B. Cloud Workstations — yönetilen, yeniden üretilebilir, kalıcı, VPC içinde kalan geliştirme ortamları sağlar; Cloud Shell ise bir saatlik hareketsizlikten sonra sonlanan, geçici, oturum başına bir yönetim kabuğudur ve bir ekibin birincil geliştirme ortamı olması amaçlanmamıştır
