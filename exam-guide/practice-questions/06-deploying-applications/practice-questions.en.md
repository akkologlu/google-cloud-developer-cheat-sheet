...
B) Yes, but only if the packages are also deployed to GKE
C) No, because Assured OSS was superseded by Binary Authorization
D) No — Assured OSS only covers Java and Python open-source packages
...
A) Cloud Build's verifiable build metadata
B) Artifact Analysis vulnerability scan results
...
B) Cloud Deploy — it blocks vulnerable images from being promoted to any target environment
C) Artifact Analysis — it scans on-demand and automatically, and keeps monitoring stored images for new vulnerabilities, but it does not by itself block anything from being deployed
...
B) No — sequenced deployments with approval/rollback must be scripted manually with `gcloud`; no managed service offers this
C) Yes, but only if Binary Authorization is disabled first
D) No — Cloud Build's job ends at building, testing, and pushing the image to Artifact Registry; sequencing deployments across target environments with one-click approval and rollback is Cloud Deploy's job
...
A) No — Software Delivery Shield is an umbrella term for the combination of Assured OSS, Cloud Build's build metadata, Artifact Analysis, Cloud Deploy, Binary Authorization, and GKE/Cloud Run runtime security; it is not a single standalone tool you switch on
B) Yes — it is a single managed API that replaces Assured OSS, Cloud Build, Artifact Analysis, Cloud Deploy, and Binary Authorization
...
B) Containers virtualize hardware exactly like VMs do, they just boot faster because of caching
C) Containers virtualize at the operating system level, using process isolation and namespaces on a single shared kernel; unlike a VM, a container does not carry its own OS copy
...
B) `source` and `target`
C) `trigger` and `tag`
D) `name` (the container invoked for that step) and `images` (the name of the image produced by the build)
...
A) Cloud Build mounts the `/workspace` directory into every step's container, and files placed there persist and are available to subsequent steps
B) Each step manually uploads its output to a Cloud Storage bucket configured in the pipeline
