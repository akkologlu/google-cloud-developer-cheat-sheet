...
B) Evet, ama sadece paketler GKE'ye de dağıtılıyorsa
C) Hayır, çünkü Assured OSS'in yerini Binary Authorization aldı
D) Hayır — Assured OSS yalnızca Java ve Python açık kaynak paketlerini kapsar
...
A) Cloud Build'in doğrulanabilir build metadata'sı
B) Artifact Analysis güvenlik açığı tarama sonuçları
...
B) Cloud Deploy — güvenlik açığı bulunan imajların herhangi bir hedef ortama terfi etmesini engeller
C) Artifact Analysis — hem talep üzerine hem otomatik tarama yapar ve depodaki imajları yeni güvenlik açıkları için sürekli izlemeye devam eder, ama tek başına hiçbir şeyin dağıtılmasını engellemez
...
B) Hayır — onay/rollback'li sıralı dağıtımlar `gcloud` ile elle script'lenmelidir; bunu sunan yönetilen bir servis yoktur
C) Evet, ama önce Binary Authorization devre dışı bırakılmalı
D) Hayır — Cloud Build'in işi imajı derlemek, test etmek ve Artifact Registry'ye push etmekle biter; hedef ortamlar arasında tek tık onay ve rollback ile sıralı dağıtım Cloud Deploy'un işidir
...
A) Hayır — Software Delivery Shield, Assured OSS, Cloud Build'in build metadata'sı, Artifact Analysis, Cloud Deploy, Binary Authorization ve GKE/Cloud Run çalışma zamanı güvenliğinin birleşimi için kullanılan bir şemsiye terimdir; tek başına açtığın tek bir araç değildir
B) Evet — Assured OSS, Cloud Build, Artifact Analysis, Cloud Deploy ve Binary Authorization'ın yerini alan tek bir yönetilen API'dir
...
B) Container'lar tıpkı VM'ler gibi donanımı sanallaştırır, sadece önbellekleme (caching) sayesinde daha hızlı açılırlar
C) Container'lar, tek bir paylaşılan çekirdek (kernel) üzerinde process izolasyonu ve namespace'ler kullanarak işletim sistemi seviyesinde sanallaştırma yapar; bir VM'in aksine, container kendi OS kopyasını taşımaz
...
B) `source` ve `target`
C) `trigger` ve `tag`
D) `name` (o adım için invoke edilecek container) ve `images` (build'in ürettiği imajın adı)
...
A) Cloud Build, `/workspace` dizinini her adımın container'ına mount ediyor ve oraya konan dosyalar kalıcı olarak sonraki adımlar için kullanılabilir hale geliyor
B) Her adım kendi çıktısını, pipeline'da yapılandırılmış bir Cloud Storage bucket'ına elle yüklüyor
